...
In the second part of this document "Statistical tests for MCAR", three tests are performed to show if an incomplete dataset follows Missing Completely At Random (MCAR) mechanism. These three tests are independently used on several generated incomplete datasets with different mechanisms, in order to show their capacity.

# Generate data

## Complete data

We generate a complete data set $(Y1, Y2, Y3, Y4, Y5, Y6, Y7, Y8)$ with $(Y1, Y2, Y3) \sim \mathcal{N}(\mu_1, S1)$, (Y4, Y5)~N(u2,S2), Y6~P(\lambda) and Y7,Y8~Binomial(prob). 
