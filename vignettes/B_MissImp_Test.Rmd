...
i <- 1
result <- list()
for (miss_prop in c( 0.3, 0.5)) { #0.005, 0.01, 0.03, 0.05, 0.07, 0.1,
  print(miss_prop)
  miss_prop_real <- c()
  ls_mse <- c()
  ls_test_mse <- c()
...
```{r rd_write_csv}
library(ggplot2)
# write.csv(result,"test1_MNAR2_Forest_boot_result.csv")
res_Ranger <- read.csv("test1_MAR2_Ranger_boot_result.csv")
res_EM <- read.csv("test1_MAR2_EM_boot_result.csv")
res_kNN <- read.csv("test1_MAR2_kNN_boot_result.csv")
res_Forest <- read.csv("test1_MAR2_Forest_boot_result.csv")
res_PCA <- read.csv("test1_MAR2_PCA_boot_result.csv")
res_MIRanger <- read.csv("test1_MAR2_MIRanger_boot_result.csv")
res_MIEM <- read.csv("test1_MAR2_MIEM_boot_result.csv")
res_MICE <- read.csv("test1_MAR2_MICE_boot_result.csv")
...
p2 <- ggplot(predframe, aes(x = miss_perc)) +
  ...
  geom_ribbon(data = predframe, aes(ymin = lwr_PCA, ymax = upr_PCA, fill = "PCA"), alpha = 0.3) +
  geom_line(aes(y = scaled_MSE_MIEM, colour = "MIEM")) +
  geom_point(aes(y = scaled_MSE_MIEM, colour = "MIEM")) +
  geom_ribbon(data = predframe, aes(ymin = lwr_MIEM, ymax = upr_MIEM, fill = "MIEM"), alpha = 0.3) +
