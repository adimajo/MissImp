...
```{r rd_write_csv}
library(ggplot2)
# write.csv(result,"test1_MAR1_KNN_boot_result.csv")
res <- read.csv("test_MAR1_Ranger_boot_result.csv")
x <- res$miss_perc
y1 <- res$scaled_MSE
y2 <- res$scaled_MSE + 1.96*res$Var_scaled_MSE
y3 <- res$scaled_MSE - 1.96*res$Var_scaled_MSE
#
# plot(x, y1, type = "n", ylim = range(c(y1, y2)), xlab = "", ylab = "")
# lines(x, y1, col = "blue")
# lines(x, y2, col = "red")
...
predframe <- with(res,data.frame(miss_perc,
                                ...
                                lwr=scaled_MSE-1.96*Var_scaled_MSE,
                                upr=scaled_MSE+1.96*Var_scaled_MSE))
