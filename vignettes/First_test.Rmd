...
col_cat <- c(7,8)
col_dis <- c(6)
n_resample <- 4#2*round(log(nrow(df_complete)))
n_df <- 5
maxiter_tree <- 3
maxiter_pca <- 50
maxiter_mice <- 5
ncp_pca <- 5#round(ncol(df_complete)/2)
learn_ncp <- FALSE
num_mi <- 3
...
```{r write_csv}
write.csv(result,"test_MAR1_MIPCA_boot_result.csv")
...
```{r write_csv}
write.csv(result,"test_MAR1_PCA_boot_diff_df_result.csv")
read.csv("test_MAR1_MIPCA_boot_result.csv")
```
