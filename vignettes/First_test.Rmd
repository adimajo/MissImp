...
```{r}
res=single_imp(df=df, imp_method=imp_method, resample_method=resample_method,
...
for(df in ls_boot){
  ...
  pca = imputeFAMD_mod(df, ncp = n_opt, maxiter = 100)
...
for(df in ls_jack){
  ...
  pca = imputeFAMD_mod(df, ncp = 3, maxiter = 100)
  ls.imp.pca.onehot[[i]] = data.frame(pca$tab.disj)
  ls.imp.pca.fact[[i]] = pca$completeObs
  i = i + 1
}
...
pca_full = imputeFAMD_mod(df_full, ncp = 3, maxiter = 20)
...
for(df in ls_jack){
  ...
  #Input: an incomplete dataset
  #Outout: an imputed dataset (onehot or factor for categorical)
  imp.forest = missForest_mod(xmis = df, maxiter = 5, ntree = 100, dict_cat = dict_name_cat)
...
df_full = df_with_mv
df_full = factor_encode(df_with_mv, idx_col_cat)
imp.forest_full = missForest_mod(xmis = df_full, maxiter = 5, ntree = 100, dict_cat = dict_name_cat)
