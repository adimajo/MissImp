...
missRanger_mod <- function(data, formula = . ~ ., pmm.k = 0L, maxiter = 10L, seed = NULL, 
  ...
  while (crit && j <= maxiter) {
    ...
    for (v in visitSeq) {
      ...
      if (length(completed) == 0L) {
        ...
        fit <- ranger::ranger(formula = reformulate(completed, response = v), 
  ...
  for (v in visitSeq) {
    ...
    if (length(completed) == 0L) {
      ...
      if(exist_cat){
        if(v %in% name_cat){
          fit.disj <- ranger::ranger(formula = reformulate(completed, response = v),
                                     data = data[!v.na, union(v, completed), drop = FALSE],
                                     case.weights = case.weights[!v.na], probability=TRUE)
          pred.disj <- predict(fit.disj, data[v.na, completed, drop = FALSE])$predictions
          data.disj[v.na, dict_cat[[v]]] <- if (pmm.k) pmm(xtrain = fit.disj$predictions,
                                                           xtest = pred.disj,
                                                           ytrain = data.disj[[v]][!v.na],
                                                           k = pmm.k) else pred.disj
        }
        else{
          data.disj[v.na, v] = if (pmm.k) pmm(xtrain = fit$predictions, 
                                              xtest = pred, 
                                              ytrain = data[[v]][!v.na], 
                                              k = pmm.k) else pred
        }
