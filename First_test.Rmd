...
suppressPackageStartupMessages(require(MissMech)) # removed from the CRAN repository.
#suppressPackageStartupMessages(require(BaylorEdPsych)) # removed from the CRAN repository.
#suppressPackageStartupMessages(require(mvnmle)) # cannot find 
suppressPackageStartupMessages(require(rlist))
...
suppressPackageStartupMessages(require(missMethods))
suppressPackageStartupMessages(require(gbutils))
...
suppressPackageStartupMessages(require(uwo4419))
...
for(df in ls_boot){
  ...
  pca = imputeFAMD(df, ncp = 3, maxiter = 200)
  ls.imp.pca[[i]] = pca$comp
  i = i + 1
}
...
```{r}
combine_boot(ls.imp.pca,col_con=c(1:5),col_dis=c(6),col_cat=c(7:8), num_row_origin = 10000)
```
...
```{r}
combine_jack = function(ls_df, col_con, col_dis, col_cat, num_row_origin){
...
imp.forest = missForest(xmis = list_df.mix[[2]], maxiter = 20, ntree = 100)
```
...
```{r}
list_df.mix[[2]] = factor_encode(list_df.mix[[2]], idx_col_cat)
ncp.pca = estim_ncpFAMD(list_df.mix[[2]], method.cv="Kfold", verbose=T)$ncp
pca = imputeFAMD(list_df.mix[[2]], ncp = 3)
imp.pca = pca$comp
```
